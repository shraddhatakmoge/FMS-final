...
export default function StaffManagementDashboard() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <header className="sticky top-0 z-10 border-b bg-white/70 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center gap-3">
          <div className="flex items-center gap-2">
            <div className="h-9 w-9 rounded-xl bg-red-600 grid place-items-center text-white font-bold">S</div>
            <span className="font-semibold">Shorat Innovations</span>
          </div>
      ...
      <main className="mx-auto max-w-7xl p-4 grid gap-6">
        {/* Breadcrumb + Title */}
        ...
        <section className="grid gap-4 md:grid-cols-5">
          ...
          <StatCard title="Roles" value={totals.byRole.reduce((a, r) => a + (r.count > 0 ? 1 : 0), 0)} icon={IdCard} footer={totals.byRole.map(r => `${r.role}: ${r.count}`).join(" · ")} />
          <StatCard title="Franchises" value={totals.byFranchise.length} icon={Building2} footer={totals.byFranchise.map(f => `${f.name}: ${f.count}`).join(" · ")} />
          <StatCard title="Leave Requests" value={leaveSummary.list.length} icon={CalendarDays} footer={`A:${leaveSummary.counts.Approved} · P:${leaveSummary.counts.Pending} · R:${leaveSummary.counts.Rejected}`} />
        </section>
